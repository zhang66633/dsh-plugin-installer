...
/**
 * Required service: the skill registry. The web-surface services are optional
 * (ctx.get) so this plugin keeps working in non-web profiles; their store
 * features simply stay dormant there.
 */
...
/**
 * Mount the bundled skill provider and, on the web surface, the store routes
 * and the prompt section.
 * @param ctx - context carrying skills; optionally webServer, systemPrompt, agents.
 */
export function apply(ctx) {
  ...
  // 2) store surface — optional services, dormant outside the web profile
