...
/**
 * Required services: the skill registry, the web surface, and the Loader
 * (to mark already-installed plugins in the store). The bundle is
 ...
 */
export const inject = ['skills', 'webServer', 'systemPrompt', 'agents', 'loader']
...
/** Install/update-request message the agent receives for a store button click. */
function installRequestText(plugin, version = '') {
  const action = version ? '更新' : '安装'
  const target = version ? `到版本 ${version} ` : ''
  return `用户点击了插件商店里的「${action}」按钮，请求${action}插件：${plugin}（GitHub 仓库 https://github.com/${plugin}）${target}。
请按 dsh-plugin-installer 技能的流程完成${action}：
1. 确认来源与插件结构（package.json / dsh.bundle / 是否有 lib 或需要构建）；
2. 选择安装路径（npm 已发布则钉精确版本${version ? `（目标 ${version}）` : ''}直装；否则 clone 源码 + 共享依赖层 + link 注册）；
...
/** Package names of currently mounted non-group, non-disabled loader entries. */
function installedModuleNames(loader) {
  const names = new Set()
...
/** Resolve one installed entry's package.json version; '' when unresolvable. */
function installedVersion(moduleName) {
  try {
    const pkgPath = requirePkg.resolve(`${moduleName}/package.json`)
...
/** True when two package.json versions differ and both are non-empty. */
...
export function apply(ctx) {
  ...
  ctx.effect(() => webServer.register({
      ...
      handler: (_req, res) => {
        const doc = JSON.parse(readFileSync(join(DATA_DIR, 'store.json'), 'utf8'))
        // Annotate installed state: a plugin is "installed" when a mounted
        // entry's package basename matches the repo basename (e.g. modlens).
        // Installed entries additionally carry their live version and, when
        // it differs from the catalog's latest_version, update_available.
        const installed = installedModuleNames(ctx.loader)
        for (const plugin of doc.plugins ?? []) {
          const base = plugin.name.split('/').pop().toLowerCase()
          const matched = [...installed].find((m) => m.split('/').pop().toLowerCase() === base)
          plugin.installed = matched !== undefined
          if (matched !== undefined) {
            plugin.installed_version = installedVersion(matched)
            plugin.update_available = versionDiffers(
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        ...
        // Version is optional: present = update request, absent = install.
        const rawVersion = payload && typeof payload.version === 'string' ? payload.version.trim() : ''
        const version = /^[A-Za-z0-9][A-Za-z0-9._+-]{0,39}$/.test(rawVersion) ? rawVersion : ''
        ...
        agent.followup({
          content: [{ type: 'text', text: installRequestText(plugin, version) }],
          source: { kind: 'user' },
        })
        json(res, 200, { ok: true, queued: true, plugin, version: version || undefined })
      },
